...
with(Kdat, plot(Longitude, Latitude, col=color.of(Kdat$K.Trial), pch=16, axes=F, xlab="", ylab="",
                main="K.Trial"))
...
# Caluclate distances - this takes aound 30 seconds on my laptop
...
par(mfrow=c(2,1))
par(mai=c(0.1, 0.1, 0.5, 0.8))
with(coefs, plot(s1, s2, col=color.of(coefs[, "Intercept"]), 
                 pch=20, axes=F, xlab="", ylab=""))
legend.col(col = rainbow(100), lev = coefs[, "Intercept"])
with(coefs, plot(s1, s2, col=color.of(coefs[, "K.Trial"]), 
              pch=20, axes=F, xlab="", ylab=""))
legend.col(col = rainbow(100), lev = coefs[, "K.Trial"])
```

It is hard to compare the respopnse to K with varying intercepts, so perfrom GWR with a fixed intercept.
...
par(mfrow=c(2,1))
par(mai=c(0.1, 0.1, 0.5, 0.8))
with(coefs2, plot(s1, s2, col=color.of(coefs2[, "K.Trial"]), 
                 pch=20, axes=F, xlab="", ylab=""))
legend.col(col = rainbow(100), lev = coefs2[, "K.Trial"])
```
